...
matching_dict = {
    ...
    "social_interaction_name": [[social_interaction_domins, social_interaction_slots], "源社交", "目标社交"]
}
q1 = [
...
def task1_gen(split, zero_shot=False):
    ...
    for instance in data:
        ...
        context = sep.join(["请根据对话内容，从0、1、2中选择对话中的[MASK]处应填入的选项。", "对话内容: " + c, choice_str,"正确的选项是："])
        answer = str(choices.index(original_phrase))
...
def task5_gen(split, zero_shot=False):
    ...
    for instance in data:
        ...
        print(type_name)
        if type_name != "social_interaction_name":
            ...
            if 'entity_name' in list(instance.keys()):
                ...
                choice_str = choice_str[:-1]
            # c = " <utt> ".join(instance["dialogue_content"])
            c = " ".join(instance["dialogue_content"])
            context = sep.join(["请根据对话内容，从0、1、2……这些领域中选择"+source+"和"+target+"的关系属于的领域。", source+": " + instance[type_name], target+": " + instance["value"], "对话内容: " + c, choice_str,"正确的领域是："])
            answer = str(choices.index(instance["domin"]))
            line = {"input": context, "output": answer}
            f.write(json.dumps(line, ensure_ascii=False) + "\n")
    f.close()
...
def task6_gen(split, zero_shot=False):
    ...
    for instance in data:
        ...
        # c = " <utt> ".join(instance["dialogue_content"])
        c = " ".join(instance["dialogue_content"])
        context = sep.join(["请根据对话内容，从0、1、2……这些类型中选择"+source+"和"+target+"的关系属于的类型。", source+": " + instance[type_name], target+": " + instance["value"], "对话内容: " + c, choice_str,"正确的类型是："])
        answer = str(choices.index(instance["slot"]))
        line = {"input": context, "output": answer}
        f.write(json.dumps(line, ensure_ascii=False) + "\n")
    f.close()

...
if __name__ == '__main__':
    ...
    # model_family = {'rob': 'roberta', 'robsq': 'roberta', 'span': 'bert'}
    # model_id = {'rob': 'chinese-roberta-wwm-ext', 'robsq': 'roberta-base-sqaud', 'span': 'spanbert-squad'}
    # model_exact_id = {'rob': 'hfl/chinese-roberta-wwm-ext', 'robsq': 'deepset/roberta-base-squad2', 
    #                   'span': 'mrm8488/spanbert-finetuned-squadv2'}
    ...
    x_train = [json.loads(line) for line in open(args.train_file).readlines()]
    x_dev = [json.loads(line) for line in open(args.validation_file).readlines()]
    x_test = [json.loads(line) for line in open(args.test_file).readlines()]
    # x_train = json.load(open(args.train_file))
    # x_dev = json.load(open(args.validation_file))
    # x_test = json.load(open(args.test_file))

    max_q_length, max_c_length, max_a_length = 50, 400, 50
    
    save_dir    = 'outputs/' + model_id[model] + '/task3' + '/'
    result_file = 'outputs/' + model_id[model] + '/task3' + '_results.txt'
    prediction_file   = 'outputs/' + model_id[model] + '/task3' + '_test_predictions.pkl' 

...
import sys
sys.path.append('../../')
from data_preprocessing import Task3_Dataloader, get_dataLoader
...
def train_loop(args, dataloader, model, optimizer, lr_scheduler, epoch, total_loss):
    ...
    model.train()
    for step, (X, start_pos, end_pos) in enumerate(dataloader, start=1):
        X, start_pos, end_pos = X.to(args.device), start_pos.to(args.device), end_pos.to(args.device)
        
        # batch_data = to_device(args, batch_data)
        # X, start_pos, end_pos = batch_data['batch_inputs'], batch_data['start_positions'], batch_data['end_positions']
        # X, start_pos, end_pos = X.to(args.device), start_pos.to(args.device), end_pos.to(args.device)
...
if __name__ == '__main__':
    ...
    model = AutoModelForQuestionAnswering.from_pretrained(args.model_checkpoint).to(args.device)
    # Training
    if args.do_train:
        train_dataset = Task3_Dataloader(args.train_file)
        dev_dataset = Task3_Dataloader(args.dev_file)
        print(f'train set size: {len(train_dataset)}')
        print(f'valid set size: {len(dev_dataset)}')
    ...
    if args.do_test:
        test_dataset = Task3_Dataloader(args.test_file)

...
def get_args():
    ...
    parser.add_argument("--chat", default=True, action="store_true",
    ...
    parser.add_argument("--use_10b", default=False, action="store_true",
                        help="10B-GLM")
    parser.add_argument("--use_335m", default=False, action="store_true",
                        help="335m-GLM")
...
def get_text(args,text,model,tokenizer):
    if args.chat:
        ...
        if args.test:
            ...
            input_ids = tokenizer(text.replace('[MASK]',''), return_tensors="pt").input_ids.cuda()
        outputs = model.generate(input_ids, max_new_tokens=20, do_sample=True, top_k=30, top_p=0.85, temperature=0.35,
...
def read_5shot(pathfile,choices,n):
    demostrations = ''
    ...
    if n>0:
        for i in range(len(questions)):
            ...
            if args.use_10b or args.use_335m:
                ...
                questions[i] = '请阅读以下选择题并给出正确选项，不要解释原因。请只给出答案的序号。'+demonstration + questions[i]

    else:
        for i in range(len(questions)):
            questions[i] = '请阅读以下选择题并给出正确选项，不要解释原因。请只给出答案的序号。' + questions[i]
    # else:
    #     for i in range(len(questions)):
...
if __name__ == "__main__":
    ...
    if args.use_tju:
        ...
        path = '/data/lc/mmm_cn/datammm/'
        dev_path = '/data/lc/mmm_cn/datammm_dev/'
    else:
        # path = 'datammm_untest/'
        path = 'datammm/'
        dev_path = 'datammm_dev/'
    # pathfile = 'datammm/primary Chinese.xlsx'
    ...
    if args.chat:
        if args.use_tju:
            tokenizer = AutoTokenizer.from_pretrained("/data/lc/PLM/glm-6b-chat/", trust_remote_code=True)
            model = AutoModel.from_pretrained("/data/lc/PLM/glm-6b-chat/", trust_remote_code=True).half().cuda()
        else:
            tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/PLM/glm-6b-chat", trust_remote_code=True)
            model = AutoModel.from_pretrained("/root/autodl-tmp/PLM/glm-6b-chat", trust_remote_code=True).half().cuda()
        directory = "output_mmm_glm6b_newinst"
    else:
        if args.use_10b:
            # 10B
            tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/PLM/glm-10b", trust_remote_code=True)
            model = AutoModelForSeq2SeqLM.from_pretrained("/root/autodl-tmp/PLM/glm-10b", trust_remote_code=True)
            model = model.half().cuda()
            directory = "output_mmm_glm10b_newinst"
            ...
            model = model.cuda()
            directory = "output_mmm_glm335m_inst"

...
def get_args():
    ...
    parser.add_argument("--task_name", default="", action="store_true",
                       help="Whether to use BMInf")
...
def generate_answer(args,text,model,tokenizer):
    ...
    #         answer, history = model.chat(tokenizer, text.replace('[MASK]',''), history=[])
        ...
        try:
            ...
            answer = tokenizer.decode(outputs[0].tolist()).split(' <|startofpiece|> ')[1].split(' \\n')[0]
        except:
            answer = 'E'
        a = 7
...
if __name__ == "__main__":
    ...
    with open(output_path,'a') as f:
        ...
        for i,question in enumerate(inputs[:270]):
            print('*'*20, i , '*'*20)
    ...
    with open(metrics_path,'w') as f:
        ...
        f.write('correct_count: {}, total_count: {}'.format(correct_count, len(answers)))
